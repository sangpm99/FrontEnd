...
import FrameLogin from "../Frame/FrameLogin";
import {Button, Form, Input} from "antd";
import {FontAwesomeIcon} from "@fortawesome/react-fontawesome";
import {faKey} from "@fortawesome/free-solid-svg-icons";

function VerifyFormForgotPassword({onVerifyForgotPasswordSuccess}) {
    const [code, setcode] = useState('');
    const [load, setLoad] = useState(false);

    const [statusCode, setStatusCode] = useState(null);

    const apiUrl = "http://localhost:8080/forgotpassword/verifycode";
    ...
    return (
        <FrameLogin>
            <p className="text-3xl w-full text-center py-5">QUÊN MẬT KHẨU</p>
            <Form
                className="w-full"
                name="basic"
                style={{
...
export default VerifyFormForgotPassword;